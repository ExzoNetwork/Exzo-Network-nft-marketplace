...
const fetchContract = (signerOrProvider: ethers.providers.JsonRpcSigner | ethers.providers.JsonRpcProvider): ethers.Contract => new ethers.Contract(MarketAddress, MarketAddressAbi, signerOrProvider);
...
interface NFTContextInterface {
  ...
  // eslint-disable-next-line no-unused-vars
  createNFT: (formInput: IFormInput, fileUrl: string, router: NextRouter) => Promise<void>;
  fetchNFTs: () => Promise<IFormattedNFT[]>;
  // eslint-disable-next-line no-unused-vars
  fetchMyNFTsOrListedNFTs: (type: 'fetchItemsListed' | 'fetchMyNFTs') => Promise<IFormattedNFT[]>;
  // eslint-disable-next-line no-unused-vars
  buyNFT: (nft: IFormattedNFT) => Promise<void>;
  // eslint-disable-next-line no-unused-vars
  createSale: (url: string, formInputPrice: string, id?: string, isReselling?: boolean) => Promise<void>;
...
export const NFTProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const createSale = async (url:string, formInputPrice: string, id?: string, isReselling = false) => {
  ...
  const createNFT = async (formInput:IFormInput, fileUrl: string, router: NextRouter) => {
  ...
  const fetchNFTs = async (): Promise<IFormattedNFT[]> => {
    ...
    const items = await Promise.all((data as IRawNFTData[]).map(async ({ tokenId, seller, owner, price: unformattedPrice }) => {
      const tokenURI = await contract.tokenURI(tokenId);
      const { data: { image, name, description } } = await axios.get<INFTMetadata>(tokenURI);
  ...
  const fetchMyNFTsOrListedNFTs = async (type: 'fetchItemsListed' | 'fetchMyNFTs'): Promise<IFormattedNFT[]> => {
    setIsLoadingNFT(false);
  ...
  return (
    <NFTContextProvider value={{ nftCurrency, connectWallet, currentAccount, uploadToIPFS, createNFT, fetchNFTs, fetchMyNFTsOrListedNFTs, buyNFT, createSale, isLoadingNFT }}>
      {children}
    </NFTContextProvider>
  );
};
