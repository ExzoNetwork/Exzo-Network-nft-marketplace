...
const NFTDetails: NextPage = () => {
  const { currentAccount, nftCurrency, buyNFT, isLoadingNFT } = useCurrentNFTContext();
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [nft, setNft] = useState<IFormattedNFT>({ image: '', tokenId: 0, name: '', owner: '', price: '', seller: '', description: '', tokenURI: '' });
  ...
  useEffect(() => {
    ...
    setNft(router.query as unknown as IFormattedNFT);
    setIsLoading(false);
  }, [router.isReady]);
